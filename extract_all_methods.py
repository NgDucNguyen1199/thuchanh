...
try:
    ...
    methods_tried.append("pypdf: SUCCESS")
except Exception as e:
    print(f"✗ FAILED: {type(e).__name__}: {e}")
    methods_tried.append(f"pypdf: FAILED - {type(e).__name__}: {e}")
...
try:
    ...
    print(f"✗ FAILED: {type(e).__name__}: {e}")
    methods_tried.append(f"pdfplumber: FAILED - {type(e).__name__}: {e}")
...
try:
    ...
    if result.returncode == 0 and result.stdout:
        ...
        methods_tried.append("pdftotext: SUCCESS")
    else:
        print(f"✗ FAILED: Return code {result.returncode}")
        if result.stderr:
            print(f"Error: {result.stderr[:500]}")
        methods_tried.append(f"pdftotext: FAILED - Return code {result.returncode}")
except Exception as e:
    print(f"✗ FAILED: {type(e).__name__}: {e}")
    methods_tried.append(f"pdftotext: FAILED - {type(e).__name__}: {e}")
...
try:
    ...
    if result.returncode == 0 and result.stdout:
        ...
        methods_tried.append("mutool: SUCCESS")
    else:
        print(f"✗ FAILED: Return code {result.returncode}")
        if result.stderr:
            print(f"Error: {result.stderr[:500]}")
        methods_tried.append(f"mutool: FAILED - Return code {result.returncode}")
except Exception as e:
    print(f"✗ FAILED: {type(e).__name__}: {e}")
    methods_tried.append(f"mutool: FAILED - {type(e).__name__}: {e}")
...
try:
    ...
    if result.returncode == 0 and result.stdout:
        ...
        methods_tried.append("tesseract: SUCCESS")
    else:
        print(f"✗ FAILED: Return code {result.returncode}")
        if result.stderr:
            print(f"Error: {result.stderr[:500]}")
        methods_tried.append(f"tesseract: FAILED - Return code {result.returncode}")
except Exception as e:
    print(f"✗ FAILED: {type(e).__name__}: {e}")
    methods_tried.append(f"tesseract: FAILED - {type(e).__name__}: {e}")

print("\n" + "="*80)
print("SUMMARY OF ALL METHODS TRIED:")
print("="*80)
